...
const HospitalListModal = ({ visible, hospitals, onClose, onHospitalSelect }) => {
       ...
       <View style={styles.modalContainer}>
            ...
            >
              ...
              <View style={styles.headerContainer}>
                ...
                />
                <Text style={{ flex: 1, ...FONTS.h2, fontSize: SIZES.body2 }}>Hospitals/ Clinics | Details</Text>  
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity style={styles.itemContainer} onPress={() => handleHospitalSelect(item)}>
                    ...
                    <View style={styles.labelContainer}>
                      <Image source={icons.hospitals} style={styles.icon} />
                      <Text style={{ ...FONTS.body3, fontSize: SIZES.body3, }}>{item.address}</Text>
                    </View>
                    <View style={styles.labelContainer}>
                      <Image source={icons.call1} style={styles.icon} />
                      <Text style={{ ...FONTS.body3, fontSize: SIZES.body3, }}>{item.contact}</Text>
                    </View>
                    <View style={styles.labelContainer}>                                           
                      <Image source={icons.time} style={styles.icon} />
                      <View>
                        <Text style={{ ...FONTS.body3, fontSize: SIZES.body3, color: COLORS.green }}>Open 24hs</Text>
                      </View>
                    </View>
                    {/* Display open/closed status */}
                    {/* <Text style={{ ...FONTS.h3, fontSize: SIZES.body3, color: isHospitalOpen(item.openingTime, item.closingTime) ? COLORS.green : COLORS.red }}>
                      {isHospitalOpen(item.openingTime, item.closingTime) ? 'Open' : 'Closed'}
                    </Text> */}
                  </TouchableOpacity>
                )}
              />
